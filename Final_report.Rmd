...
team_stats1 <- team_stats1 %>% 
  select(team, f_tr, x3p_ar, ts_percent, e_fg_percent, ft_fga)
...
tm_stats <- tm_stats %>% 
  select(!c(g, rk, team, team_id, salary))
...
player_stats <- left_join(player_stats, player_salary)
...
player_stats <- left_join(player_stats, players)
...
ggplot(NULL, aes(tm_stats$pts, r_hat)) +
  geom_point()
```
